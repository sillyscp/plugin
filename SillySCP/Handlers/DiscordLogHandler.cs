    ...
    public class DiscordLogHandler : IRegisterable
    {
        ...
        public void OnPlayerLeave(LeftEventArgs ev)
        {
            ...
            if (!string.IsNullOrEmpty(ev.Player.Nickname) && !Round.IsEnded && Round.IsStarted)
                DiscordBot.Instance.ConnectionChannel.SendMessageAsync(
                    $"Player `{ev.Player.Nickname}` (`{ev.Player.UserId}`) has left the server");
        }
        ...
        private void OnPlayerDying(DyingEventArgs ev)
        {
            ...
            if (ev.Attacker != null && ev.Player != ev.Attacker)
            {
                ...
                if (ev.Player.Role == RoleTypeId.ClassD || ev.Player.Role == RoleTypeId.Scientist ||
                    ev.Player.Role == RoleTypeId.FacilityGuard)
                {
                    cuffed = ev.Player.IsCuffed;
                }

                text +=
                    $"Player `{ev.Player.Nickname}` (`{ev.Player.Role.Name}`){(cuffed ? " **(was cuffed)**" : "")} has been killed by `{ev.Attacker.Nickname}` as `{ev.Attacker.Role.Name}`";
